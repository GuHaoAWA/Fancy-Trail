...
@OnlyIn(Dist.CLIENT)
public class AfterImageTrailParticle extends AnimationTrailParticle {
    ParticleRenderType fastAfterImageType = FTRenderType.rapidSlashRenderType(
            trailInfo.texturePath()
    );

    protected AfterImageTrailParticle(ClientLevel level, LivingEntityPatch<?> owner, Joint joint, AssetAccessor<? extends StaticAnimation> animation, TrailInfo trailInfo) {
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(@NotNull SimpleParticleType typeIn, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            ...
            if (result.playable()) {
                return new AfterImageTrailParticle(level, entitypatch, entitypatch.getArmature().searchJointById(jointId), animation, result);
            } else {
                return null;
            }
        }
