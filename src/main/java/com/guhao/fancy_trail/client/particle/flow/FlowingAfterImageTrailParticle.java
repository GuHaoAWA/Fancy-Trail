...
@OnlyIn(Dist.CLIENT)
public class FlowingAfterImageTrailParticle extends FlowingAnimationTrailParticle {
    ParticleRenderType fastAfterImageType = FTRenderType.rapidSlashRenderType(
            trailInfo.texturePath()
    );
    protected FlowingAfterImageTrailParticle(ClientLevel level, LivingEntityPatch<?> owner, Joint joint, AssetAccessor<? extends StaticAnimation> animation, TrailInfo trailInfo) {
    ...
    public void render(VertexConsumer pBuffer, Camera pRenderInfo, float pPartialTicks) {
        ...
        FTRenderType.rapidSlashRenderType(
                trailInfo.texturePath()).callPipeline();
    }


    public @NotNull ParticleRenderType getRenderType() {
        return fastAfterImageType;
    }
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(@NotNull SimpleParticleType typeIn, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            ...
            if (result.playable()) {
                return new FlowingAfterImageTrailParticle(level, entitypatch, entitypatch.getArmature().searchJointById(jointId), animation, result);
            } else {
                return null;
            }
        }
