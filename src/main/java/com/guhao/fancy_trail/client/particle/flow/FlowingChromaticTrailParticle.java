...
@OnlyIn(Dist.CLIENT)
public class FlowingChromaticTrailParticle extends FlowingAnimationTrailParticle {


    protected FlowingChromaticTrailParticle(ClientLevel level, LivingEntityPatch<?> owner, Joint joint, AssetAccessor<? extends StaticAnimation> animation, TrailInfo trailInfo) {
    ...
    public void render(VertexConsumer pBuffer, Camera pRenderInfo, float pPartialTicks) {
        ...
        FTRenderType.strongChromaticAberrationRenderType(trailInfo.texturePath()).callPipeline();

    }


    public @NotNull ParticleRenderType getRenderType() {
        return FTRenderType.strongChromaticAberrationRenderType(trailInfo.texturePath());
    }
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(@NotNull SimpleParticleType typeIn, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            ...
            if (result.playable()) {
                return new FlowingChromaticTrailParticle(level, entitypatch, entitypatch.getArmature().searchJointById(jointId), animation, result);
            } else {
                return null;
            }
        }
